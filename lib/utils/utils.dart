...
get headers => {
      ...
      "vs": "1.2031.0",  // version: 1.2024.0+2024 version de app
      "idplataforma": 
      Sistema.isAndroid ? '1': 
      Sistema.isIOS ? '2'    :
       '3',
...
Future<bool> getDeviceDetails({String uuid = ''}) async {
  ...
  if (Sistema.isAndroid) {
    ...
    modelo = build.model; // Cambié de `build.display` a `build.model`
    so = build.version.sdkInt.toString();
    iPD = build.isPhysicalDevice.toString();
    if (uuid == '') {
      pysics = generateMd5('${build.id}');
...
Future<String> obtenerLinkAgencia(
  ...
  if (promocion == null)
    url =
        '${Sistema.uriDynamic}?store=${catalogo.agencia}&catalogo=${catalogo.idAgencia}&key=${DateTime.now().millisecondsSinceEpoch}';
  else
    url =
        '${Sistema.uriDynamic}?store=${catalogo.agencia}&catalogo=${catalogo.idAgencia}&idP=${promocion.idPromocion}&key=${DateTime.now().millisecondsSinceEpoch}';

  if (isEncomiendas)
    url =
        '${Sistema.uriDynamic}?store=${catalogo.agencia}&agencia=${catalogo.idAgencia}&key=${DateTime.now().millisecondsSinceEpoch}';
  ...
  try {
    final DynamicLinkParameters parameters = DynamicLinkParameters(
      ...
      androidParameters: AndroidParameters(
          ...
          fallbackUrl: Uri.parse("https://sacaba.rantybolivia.com/instalar/"), // Enlace de fallback para Android
      ),
    ...
    link = link.replaceFirst('/', '', link.length - 1);
    ...
    print('Error al obtener el link: $err');
  }
  complet();
  return link;
}
