...
double parseDouble(String value) {
  ...
  return parsedValue ?? 0.0; // Si no puede convertir, devuelve 0.0
}
  ...
  print('Respuesta del backend: ${resp.body}');
      ...
      if (decodedResp['estado'] == 1) {
        ...
        if (decodedResp['cajeros'] != null) {
        for (var cajero in decodedResp['cajeros']) {
          CajeroModel _cajero = CajeroModel.fromJson(cajero);
print('Datos de cajero: ${cajero}');

        // Aquí es donde procesas los nuevos valores de latitud y longitud
_cajero.ltSucursal = parseDouble(cajero['lt_sucursal'].toString());
_cajero.lgSucursal = parseDouble(cajero['lg_sucursal'].toString());
_cajero.ltAgencia = parseDouble(cajero['lt_agencia'].toString());
_cajero.lgAgencia = parseDouble(cajero['lg_agencia'].toString());
  ...
  print('No se encontraron cajeros en la respuesta.');
}}
