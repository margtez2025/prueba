...
class ClienteProvider {
  ...
  final String _urlVerificarValidadCelular = 'cliente/verificar-validar-celular';
...
verificarValidadCelular(dynamic celular, String token, Function response) async {
  var client = http.Client();
  try {
    final resp = await client.post(
      ...
      headers: {
        ...utils.headers,
        'X-Firebase-AppCheck': token, // Usa el token recibido
      },
      body: {
        'idCliente': _prefs.idCliente,
        'auth': _prefs.auth,
...
validadCelular(dynamic celular, dynamic  token, {dynamic idClienteVerificar = 0}) async {
  var client = http.Client();
  try {
    final resp = await client.post(
      ...
      body: {
        'idCliente': _prefs.idCliente,
        'idClienteVerificar': idClienteVerificar == 0 ? _prefs.idCliente: idClienteVerificar.toString(),
        'auth': _prefs.auth,
        'celular': celular.toString(),
      },
    );
...
if (idClienteVerificar == 0 && decodedResp['estado'] == 1) {
  ClienteModel clienteModel = ClienteModel.fromJson(decodedResp['cliente']);
  _prefs.clienteModel = clienteModel;
  _prefs.sms = '';

  // Imprime el clienteModel para verificar que se asigna correctamente
  print('ClienteModel actualizado: $_prefs.clienteModel');
}
